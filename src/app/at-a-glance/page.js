  ...
  const getData = () => {
    fetch(`http://ec2-44-202-145-148.compute-1.amazonaws.com/api-queries/overview/5165/?type_search=${filter[filterType]}&${valueRadio === 1 ? 'quarter' : 'month'}_search=${customDate}`, {
      ...
      .then((data) => {
        ...
        setDataSpendByCarrier({
          ...dataSpendByCarrier, datasets: [
            {
              label: ' ',
        ...
        console.log(data);
        setDataShippingSpendByServiceType({
          ...dataShippingSpendByServiceType, datasets: [
            {
              label: ' ',
      ...
      <Row style={{ marginTop: '15px' }}>
        <Col span={24}>
          <Descriptions column={6}>
            {message && !['Current month', 'Year to date', 'Last 12 months'].includes(message?.[0]?.['date_range'])  && <Descriptions.Item label="Date Range">{message?.[0]['date_range']}</Descriptions.Item>}
          </Descriptions>
        </Col>
      </Row>
      <Row>
              ...
              <Row>
                ...
                <Col span={12}>
                  <Card size='small' style={{
                    ...
                    <p style={{ fontWeight: 'bold', margin: '0px', fontSize: '16px' }}>Ground {message && message[0]?.shipping_spend_by_service_type?.ground?.[1]} </p>
                    <Title level={5} style={{
                      fontWeight: 'bold',
                      margin: '0px',
